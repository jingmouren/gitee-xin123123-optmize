...
class Aberration(SimpleBacktest):
    ...
    def signal_cal(self):
        if (self.num - 1) % self.trade_date == 0:  # 每隔n个周期计算最优参数
            ...
            freq_ret = ret.apply(lambda x: x[x > 0].sum() / abs(x[x < 0].sum()))
            ret[ret > 0] = 1
            ret[ret < 0] = 0
            freq_ret = ret.mean()
            n = 2
...
stragety_name = 'Aberration_1d'  # 策略名
